---
title: "Modelling approaches for meta-analyses with dependent effect sizes in ecology and evolution: Case sudies"
...
This page is a summary of the results from the two case studies comparing different modelling approaches for meta-analyses with dependent effect sizes in ecology and evolution.
...
Now, we want to set up a vector of within-study correlations $\rho$ to test (including $\rho=0$). Then we specify the sampling error variances (the diagonal of the assumed sampling error $\mathbf{V^*}$). For each $\rho$ value we derive the assumed sampling error $\mathbf{V^*}$, fit the model with a t-distribution and using the adjusted degrees of freedom as recommended by \texttt{metafor} (see [more recommended for metafor practices here](https://wviechtb.github.io/metafor/reference/misc-recs.html)).
...
# Case study 2: Phylogenetic multilevel meta-analysis on behavioural predictability

Horváth et al. (2023) investigated whether behavioural type (mean behaviour) and behavioural predictability (within-individual variation) evolve independently or under system-specific constraints across multiple species.
Here we load and prepare the meta-analytical dataset and phylogenetic tree provided in Horváth et al. (2023). 
...
Now, let's set up the vector of within-study correlations $\rho$ that we want to test (including $\rho=0$). Then we specify the sampling error variances (the diagonal of the assumed sampling error $\mathbf{V^*}$). For each $\rho$ value we derive the assumed sampling error $\mathbf{V^*}$, fit the phylogenetic meta-analytical model with a t-distribution and using the adjusted degrees of freedom as recommended by \texttt{metafor} (see [more recommended for metafor practices here](https://wviechtb.github.io/metafor/reference/misc-recs.html)).
