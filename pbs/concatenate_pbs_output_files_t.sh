...
read -p "Enter the job number range (START-END): " JOB_RANGE

# Extract the start and end job numbers
IFS='-' read -r START_JOB_NUM END_JOB_NUM <<< "$JOB_RANGE"

# Validate input
if [[ ! "$START_JOB_NUM" =~ ^[0-9]+$ ]] || [[ ! "$END_JOB_NUM" =~ ^[0-9]+$ ]]; then
    echo "Error: Invalid job number range format. Please enter as START-END (e.g., 1-100)."
    exit 1
fi
